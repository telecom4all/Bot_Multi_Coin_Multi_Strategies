...
class DataExchange():
    def get_data_exchange_bitget(pairList, configuration, exchange, telegram_send):
        dfList = {}
    ...
    def get_data_exchange_binance(pairList, configuration, exchange, telegram_send):
        dfList = {}
        for pair in pairList:
            ...
            try :
                ...
                df = exchange.get_more_last_historical_async(pair, configuration['timeframe'], int(configuration['nbOfCandles']))
                dfList[pair.replace('/USDT','')] = df
            except :
                #Si on ne parvient à récupérer la paire à la première tentative (parfois l'api  est inaccessible) :
                #On attend X seconde(s) et on réessaye avec moins de bougies
                time.sleep(int(configuration['delay_coin']))
                try :
                    df = exchange.get_more_last_historical_async(pair, configuration['timeframe'], int(configuration['nbOfCandles'])-int(configuration['nbOfCandles']*0.25))
                    dfList[pair.replace('/USDT','')] = df
                    
                except :
                    time.sleep(int(configuration['delay_coin'])+1)
                    try :
                        df = exchange.get_more_last_historical_async(pair, configuration['timeframe'], int(configuration['nbOfCandles'])-int(configuration['nbOfCandles']*0.50))
                        ...
                        time.sleep(int(configuration['delay_coin'])+2)
                        try : 
                            df = exchange.get_more_last_historical_async(pair, configuration['timeframe'], int(configuration['nbOfCandles'])-int(configuration['nbOfCandles']*0.75))
                            dfList[pair.replace('/USDT','')] = df
                        except :
                            #Si au bout de la 3ème fois ça n'a vraiment pas fonctionné, on abandonne
                            try :
